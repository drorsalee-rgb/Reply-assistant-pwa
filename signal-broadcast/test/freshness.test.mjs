...
const { postAgeDays, checkFreshness, MAX_POST_AGE_DAYS } = require('../src/freshness.js');
...
test('that alert would have been refused', () => {
  const { stale, ageDays } = checkFreshness(INCIDENT_URL, INCIDENT_SENT);
...
test('a post from today passes', () => {
  ...
  assert.equal(checkFreshness(url, now).stale, false);
});
...
test('the boundary is inclusive up to the limit', () => {
  ...
  assert.equal(checkFreshness(justInside, now).stale, false);
  assert.equal(checkFreshness(justOutside, now).stale, true);
});
...
test('x.com and twitter.com are both understood', () => {
  ...
  for(const host of ['x.com', 'twitter.com']){
    assert.equal(checkFreshness(`https://${host}/someone/status/${id}`, now).stale, true, host);
  }
});
